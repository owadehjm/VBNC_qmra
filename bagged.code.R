#==================================================================================
### QMRA OF ECO157:H7 IN LETTUCE ### BAGGED LETTUCE ###
#==================================================================================
...
library(rpart)#For regression tree in R
...
bagged <- data.frame(P.ill_0_0 = P.ill)

a_bagged <- data.frame(P.ill_0_0 = P.ill)


bagged_scenario<- function(col1,col2){
  ...
  #Specify the harvest temperature as "baseline", 17 or 9
  harvest.T= col1 # The value can be baseline, 9 or 17
  ...
  t.Har= mc2d::rtrunc(distr = rgamma,n=iter,lsup = 8,shape= 0.3053600,rate = 0.3399035)
  # Temperature of lettuce during harvest (T.har)
  if (harvest.T == 9) {
    T.Har <- 9
  ...
  t.Tran1= rtrunclnorm(n=iter,meanlog = 1.128,sdlog = 0.459,a=0)
  ...
  T.ecool = rlnorm(meanlog = 0.380, sdlog = 0.484, n=iter)
  ...
  t.Rs= rtruncnorm(n=iter, a=0, mean = 0.108, sd = 0.019)
  
  # Retail temperature in degrees celsius (temp.R[TR], truncated normal distribution)
  # Citation: Ecosure 2008
  # Normal distribution with values truncated at 0 was used as refrigeration at retail seldome goes below zero and beyond 20.56
  ...
  T.Rd = quantile(ecdf(Rd.dat$Temperature), runif(iter))
  ...
  # Citation: Ecosure 2008
  T.bH= rtruncnorm(n=iter,mean=8.3858,sd=3.8314,a=0,b=20)
  
  # Transportation temperature in degrees celsius (temp.trans2)
  T.Tran4=1/2*(T.Rd+T.bH)
  
  ## Home storage
  ## 1. Initial prevalence (Prev0)
  ...
  # Citation Puillot 2010
  t.F= rtrunc(rweibull,n=iter, shape = 1.13,scale=2.84,lsup = 7)*24
  ...
  # f.vbnc <- ifelse(T.har == 9, 0.69 / 100,
  #                 ifelse(T.har == 17 & T.tran2 >= 6, -9.6 / 100,
  #                        ifelse(T.har == 17 & T.tran2 >= 6, 7.4 / 100, NA)))
  
  #Concentration of  STEC in the produce at after decay post cooling
  if(harvest.T=="baseline" & cold.storage.time=="None"){
    logC.p=logC.tran2
  } else{
    if(harvest.T==9 & cold.storage.time==0){
    ...
    # The produce is not processed
    logC.p= log10((10^logC.tran2)+f.vbnc)
  } 
  ...
  LogD.Rd= ifelse(T.Rd>5,R.Rd, LR.Rd)
  ...
  #Obtain this value from literature of dose response curves. Obtain this value from literature
  ...
  bagged[[paste0("P.ill_", harvest.T, "_", cold.storage.time)]] <- P.ill
  ...
  a_bagged[[paste0("P.A.ill_",harvest.T, "_", cold.storage.time)]] <- P.A.ill

  #-----------------------------------------------------------------------------
  ### Sensitivity analysis
  #-----------------------------------------------------------------------------
  # Create a data frame of the distributions in the model
  sensitivity.dat<-data.frame(as.numeric(Cw),as.numeric(VRw),as.numeric(W),as.numeric(t.hold),
                              as.numeric(T.Tran1),as.numeric(t.Tran1),as.numeric(t.cool),
                              as.numeric(T.cool),as.numeric(t.pcool),
  ...
  names(sensitivity.dat)<- c("Ec_Water","STEC_ratio_water","Water_holding_capacity","Post_irrigation_Holding_time",
                             "Transport_temp_cooling","Transport_time_Cooling","Cooling_time",
                             "cooling_temp","Distribution_time",
                             "Retail_storage_time","Temp_retail_storage","Retail_display_time",
                             "retail_display_temp","Transport_time_home",
                             "Transport_temp_home","Home_storage_time","Home_storage_temp",
                             "Harvest_temp","Harvest_time","Prob.Risk")
  ...
  rf.plot1<-ggplot(rf.data[rf.data$`%IncMSE`>1.00e-10,,], aes(x = reorder(condition, `%IncMSE`), y = `%IncMSE`)) +
    geom_bar(stat = "identity", fill = "grey") +  # Adjust fill color
    geom_text(label="A",y=0.20,x=1,size=5,family = "serif",col = "black")+
    labs(x = "Conditions", y = "% MSE",title = "Romaine hearts") +        # Label axes
  ...
  filename <- paste0(path, "/bagged_", harvest.T, "_", cold.storage.time, ".png")
